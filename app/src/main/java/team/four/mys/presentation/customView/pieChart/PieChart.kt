...
class PieChart @JvmOverloads constructor(
    ...
    private var dataList: List<Float> = listOf()
     ...
     */
    override fun setDataChart(list: List<Float>) {
        dataList = list
    ...
    private fun calculateCircleRadius(width: Int, height: Int) {
        ...
        val centerX = (circleViewWidth.toFloat() - (circleRadius * 2)) / 2

        with(circleRect) {
            left = circlePadding + centerX
            top = height / 2 - circleRadius
            right = circleRadius * 2 + circlePadding + centerX
    ...
    private fun calculatePercentageOfData() {
        totalAmount = dataList.fold(0) { res, value -> (res + value).toInt() }
